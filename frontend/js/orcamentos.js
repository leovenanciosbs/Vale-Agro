...
async function carregarOrcamentos() {
    const resposta = await fetch('/orcamentos');
...
async function buscarOrcamento(id) {
    const resposta = await fetch(`/orcamentos/${id}`);
    const dados = await resposta.json();
...
async function cancelarOrcamento(id) {
    ...
    try {
        const resposta = await fetch(`/orcamentos/${id}/status`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function removerOrcamentoHistorico(id) {
    ...
    try {
        const resposta = await fetch(`/orcamentos/${id}/remover-historico`, {
            method: 'PATCH'
        });
        const dados = await resposta.json();
