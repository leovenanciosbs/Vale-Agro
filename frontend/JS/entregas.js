...
async function carregarEntregas() {
    ...
    try {
        const resposta = await fetch(`http://localhost:3000/entregas?${params.toString()}`);
...
async function atualizarStatusEntrega(id, status) {
    ...
    try {
        const resposta = await fetch(`http://localhost:3000/entregas/${id}/status`, {
            method: 'PUT',
            headers: {
...
async function transferirEntregaParaPrazo(id) {
    ...
    try {
        const resposta = await fetch(`http://localhost:3000/entregas/${id}/transferir-a-prazo`, {
            method: 'POST'
        });
...
async function confirmarRecebimentoEntrega() {
    ...
    try {
        const resposta = await fetch(`http://localhost:3000/entregas/${id}/receber`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
